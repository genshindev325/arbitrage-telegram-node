...
async function calculateProfit(orderBooksA, orderBooksB, exchangeAName, exchangeBName) {
  const profits = [];
  ...
  for (const pair in orderBooksA) {
    if (orderBooksB[pair]) {
      ...
      const profitAB = maxVolumeAB * spreadAB;
      const profitBA = maxVolumeBA * spreadBA;
