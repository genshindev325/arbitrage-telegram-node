...
const exchanges = {
  'Gate': new ccxt.gateio({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) }),
  'Mexc': new ccxt.mexc({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) }),
  'Bybit': new ccxt.bybit({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) }),
  'Kucoin': new ccxt.kucoin({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) }),
  'Bitget': new ccxt.bitget({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) }),
  'Okx': new ccxt.okx({ 'rateLimit': 150, 'agent': new SocksProxyAgent(proxies[0]) })
}
...
async function fetchOrderBooks(exchange, pairs, exchangeBuyName, exchangeSellName, index) {
  const orderBooks = {};
  ...
  await Promise.all(
    pairs.map(async (pair, i) => {
      try {
        let orderBook = {};
        exchange.agent = new SocksProxyAgent(proxies[(index + i) % 10]);
...
async function executeArbitrageCheck(exA, exB) {
  ...
  try {
    ...
    const pairsToProcessTest = [
      // { pairs: PairsExAFExBF, exchangeA: exchangePlateA, exchangeB: exchangePlateB, exchangeAName: `${exA}-FUTURE`, exchangeBName: `${exB}-FUTURE` },
      { pairs: PairsExASExAF, exchangeA: exchangePlateA, exchangeB: exchangePlateA, exchangeAName: `${exA}-SPOT`, exchangeBName: `${exA}-FUTURE` },
      // { pairs: PairsExASExBF, exchangeA: exchangePlateA, exchangeB: exchangePlateB, exchangeAName: `${exA}-SPOT`, exchangeBName: `${exB}-FUTURE` },
      // { pairs: PairsExBSExBF, exchangeA: exchangePlateB, exchangeB: exchangePlateB, exchangeAName: `${exB}-SPOT`, exchangeBName: `${exB}-FUTURE` },
      // { pairs: PairsExBSExAF, exchangeA: exchangePlateB, exchangeB: exchangePlateA, exchangeAName: `${exB}-SPOT`, exchangeBName: `${exA}-FUTURE` }
    ]

    await Promise.all(
      pairsToProcessTest.map(async ({ pairs, exchangeA, exchangeB, exchangeAName, exchangeBName }, index) => {
        ...
        const batches = chunkify(pairs, 20);
        for (const batch of batches) {
          ...
          await Promise.all([
            orderBooksA = filterActivePairs(await fetchOrderBooks(exchangeA, batchA, exchangeAName, exchangeBName, index)),
            orderBooksB = filterActivePairs(await fetchOrderBooks(exchangeB, batchB, exchangeBName, exchangeAName, index))
          ])
...
(async function main() {
  ...
  while (true) {
    ...
    await runArbitrageChecksForRange(EXCHANGES, 0, 1);
